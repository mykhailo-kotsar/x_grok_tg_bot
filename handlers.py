...
async def cmd_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(MENU_TEXT, parse_mode=ParseMode.MARKDOWN, reply_markup=MENU_KB)
...
async def cmd_new(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bridge.is_busy():
        await update.message.reply_text("⏳ Grok is thinking. Wait or /cancel")
        return
    msg = await update.message.reply_text("🔄 Opening new chat...")
    url = await bridge.new_chat()
    if url:
        context.user_data["pending_new_chat_url"] = url
        context.user_data["active_chat_id"] = None
        await msg.edit_text("✅ New chat opened. Send your first message.")
    else:
        await msg.edit_text("❌ Failed to open new chat.")

async def cmd_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    index = store.load_index(update.effective_user.id)
    await update.message.reply_text(_fmt_index(index), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_switch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /switch N")
        return
    ...
    if not item:
        ...
        return
    msg = await update.message.reply_text(f"🔄 Switching to «{item['title']}»...")
    ok = await bridge.switch_to(item["url"])
    if not ok:
        await msg.edit_text("❌ Failed to switch. Check logs.")
        return
    ...
    if not messages:
        await msg.edit_text(f"✅ Chat «{item['title']}» is active. No history yet.")
        return
    await msg.edit_text(f"✅ Chat «{item['title']}». Loading history...")
    current = ""
    parts = []
    for m in messages:
    ...
    await update.message.reply_text(f"💬 Continue — chat «{item['title']}» is active.")

async def cmd_rename(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if not args or len(args) < 2 or not args[0].isdigit():
        await update.message.reply_text("Usage: /rename N new title")
        return
...
async def cmd_export(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("❌ Chat is empty.")
        return
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bridge.is_busy():
        bridge.cancel()
        await update.message.reply_text("⚠️ Cancel signal sent.")
    else:
        await update.message.reply_text("Grok is not currently thinking.")
